...
const GitProviderSchema = z.object({
	customGitBuildPath: z.string().min(1, "Path is required").default("/"),
	customGitUrl: z.string().min(1, {
		message: "Repository URL is required",
	}),
	customGitBranch: z.string().min(1, "Branch required"),
...
export const SaveGitProviderMonorepo = ({ monorepoId }: Props) => {
	...
	const { mutateAsync, isLoading } =
		api.monorepo.saveGitProvider.useMutation();
	...
	useEffect(() => {
		if (data) {
			form.reset({
				customGitSSHKeyId: data.customGitSSHKeyId || undefined,
				customGitBranch: data.customGitBranch || "",
				customGitBuildPath: data.customGitBuildPath || "/",
				customGitUrl: data.customGitUrl || "",
	...
	const onSubmit = async (values: GitProvider) => {
		await mutateAsync({
			customGitBranch: values.customGitBranch,
			customGitBuildPath: values.customGitBuildPath,
			customGitUrl: values.customGitUrl,
			customGitSSHKeyId: values.customGitSSHKeyId === "none" ? null : values.customGitSSHKeyId,
			monorepoId,
