...
export function createApplicationContext(
	...
	const appPorts: PortInfo[] = application.ports?.map(port => ({
		portId: port.portId,
 ...
 */
export function createComposeContext(compose: ComposeNested): EnvGenerationContext {
	const composeDomains: DomainInfo[] = compose.domains?.map(domain => ({
		domainId: domain.domainId,
		host: domain.host,
		https: domain.https,
		port: domain.port,
...
export function createProjectContext(project: {
	...
	const applications: ApplicationInfo[] = project.applications?.map(app => ({
	...
	const services: ServiceInfo[] = project.services?.map(service => ({
		id: service.id,
		name: service.name,
		type: service.type,
		appName: service.appName,
		domains: service.domains?.map(domain => ({
			domainId: domain.domainId,
			host: domain.host,
			https: domain.https,
			port: domain.port,
 ...
 */
export function extractDatabaseConnectionInfo(envVars: Record<string, string>): {
...
export function generateConnectionUrls(
	envVars: Record<string, string>,
	credentials: {
		postgres?: Record<string, {user: string; password: string; database: string}>;
		mysql?: Record<string, {user: string; password: string; database: string}>;
		mongo?: Record<string, {user: string; password: string; database: string}>;
	...
	dbInfo.redis.forEach(db => {
		...
		connectionUrls[`REDIS_${db.name}_URL`] = `redis://${auth}${db.host}:${db.port}`;
	});
